...
export default function ListingLinkForm({ onSaved }: { onSaved: () => void }) {
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState("");
  const [transaction, setTransaction] = useState("purchase");
  const [seed, setSeed] = useState<ListingObservation | null>(null);
  ...
  return <><ListingCollector onObserved={onSaved} onApply={item=>{if(busy)return;setSeed(item);setTransaction(item.fields.transaction_type||"purchase");setMessage("추출값을 적용했습니다. 부동산 유형·주소·금액·거래 상태를 확인해주세요.");}} /><section className="space-y-3 rounded-xl border bg-white p-5">
    ...
    <form onSubmit={submit} key={seed?.observation_id??"manual"}>
      <fieldset disabled={busy} className="grid gap-3 text-sm sm:grid-cols-2">
        ...
        <label>매물 이름<input name="name" defaultValue={seed?.fields.name??""} required maxLength={150} className={inputClass} /></label>
        <label>부동산 유형<select name="property_type" className={inputClass}>{[["apartment","아파트"],["officetel","오피스텔"],["row_house","연립·다세대"],["detached","단독·다가구"],["non_residential","상업·업무"],["industrial","공장·창고"],["land","토지"]].map(([v,n])=><option value={v} key={v}>{n}</option>)}</select></label>
        <label>확인한 주소<input name="address" defaultValue={seed?.fields.address??""} required maxLength={500} className={inputClass} placeholder="서울특별시 강남구 역삼동 …" /></label>
        ...
        <label>확인한 거래 상태<select name="status" defaultValue="unknown" className={inputClass}><option value="unknown">미확인</option><option value="active">거래 가능으로 표시됨</option><option value="withdrawn">철회됨</option><option value="completed">거래 완료로 표시됨</option></select></label>
